...
- настройки: обязательно должны использоваться cookies для управления сессиями
...
1) GET-запросы в веб-приложении не должны выполнять каких-либо действий от лица пользователя (все равно возможна эксплуатация запросами другого типа).
...
* токен должен храниться в базе данных (statefull метод).
* Для формирования токена необходимо использовать user id и timestamp, иначе cookie может быть использован одного пользователя может быть использваон другим пользователем в первом случае, и во втором случае для ограничения времени жизни токена для защиты от использования токена  в случае его копроментации.
...
* сервер обязан проверить на идентичность токен из cookie и токен, который прислал клиент, если оба токена совпадают, то запрос не подвергся CSRF-Атаке, в ином случае — логируем событие и отклоняем запрос.
...
* SameSite - это атрибут;
* SameSite имеет два значения:
  ** 1)Strict;
  2)Lax;
* Chrome и Opera уже сейчас поддерживает эту возможность.
